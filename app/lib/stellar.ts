...
export async function verifyStellarPayment(
    ...
    let tx: Awaited<ReturnType<typeof horizon.transactions().transaction>>;
    try {
    ...
    if (!payment) {
        throw new Error('No matching payment operation found for the given destination');
    }
 ...
 */
export async function submitStellarWithdrawal(params: WithdrawalParams): Promise<string> {
    ...
    if (!result.successful) {
        throw new Error(`Stellar submission failed: ${JSON.stringify(result.extras?.result_codes)}`);
    }
 ...
 */
export async function getLiveBalances(stellarAddress?: string | null): Promise<AssetBalance[]> {
